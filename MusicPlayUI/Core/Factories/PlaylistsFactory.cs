...
namespace MusicPlayUI.Core.Factories
{
    ...
    {
        public static async Task<PlaylistModel> CreateFavoritePlaylist()
        {
            ...
            playlist.Tracks = (await DataAccess.Connection.GetFavoriteTracks()).ToOrderedTrackModel();
            playlist.Duration = playlist.Tracks.GetTotalLength(out int _);
            return playlist;
        }

        public static async Task<PlaylistModel> CreateLastPlayedPlaylist()
        {
            PlaylistModel playlist = new PlaylistModel();
            ...
            playlist.Tracks = (await DataAccess.Connection.GetLastPlayedTracks(100)).ToOrderedTrackModel();
            playlist.Duration = playlist.Tracks.GetTotalLength(out int _);
            return playlist;
        }

        public static async Task<PlaylistModel> CreateMostPlayedPlaylist()
        {
            PlaylistModel playlist = new PlaylistModel();
            ...
            playlist.Tracks = (await DataAccess.Connection.GetMostPlayedTracks(50)).ToOrderedTrackModel();
            playlist.Duration = playlist.Tracks.GetTotalLength(out int _);
            return playlist;
        }

        public static PlaylistModel CreateRadioPlaylist(string playlistName, string description, string cover,  List<TrackModel> tracks)
        {
            return new()
            {
                Name = playlistName,
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<List<PlaylistModel>> GetConstAutoPlaylists()
        {
        ...
        public static Task<int> CreatePlaylist(this string name, string description, string cover)
        {
            if (cover.ValidPath())
            {
                ...
                ImageHelper.SaveFileToNewPath(cover, newPath);
                cover = newPath;
            }
            else
            {
        ...
        {
            return (!string.IsNullOrWhiteSpace(name), cover.ValidPath(true));
        }
