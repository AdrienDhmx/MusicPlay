...
namespace VirtualizingControls
{
    public class VirtualizingWrapPanel : VirtualizingPanelBase
    {
        ...
        protected void CalculateSpacing(Size finalSize, out double innerSpacing, out double outerSpacing)
        {
            ...
            double itemPerRow = ItemsPerRowCount;
            if(Items.Count < ItemsPerRowCount)
            {
                itemPerRow = Items.Count;
            }

            double totalItemsWidth = Math.Min(GetWidth(childSize) * itemPerRow, finalWidth);
            ...
            switch (spacingMode)
            {
                case SpacingMode.Uniform:
                    innerSpacing = outerSpacing = unusedWidth / (itemPerRow + 1);
                    break;

                case SpacingMode.BetweenItemsOnly:
                    innerSpacing = unusedWidth / Math.Max(itemPerRow - 1, 1);
                    outerSpacing = 0;
                    break;
